...
- ラボ 05 - AZURE Center for SAP ソリューションを使用してデプロイを自動化するには、このラボにデプロイされた Azure VM が存在する Azure リージョンで、次の vCPU 可用性を備えた Microsoft Azure サブスクリプションが必要です。

    - 4 x Standard_E4ds_v4 (各 4 vCPU) または 4 x Standard_D4ds_v4 (各 4 vCPU) = 8
    - 2 x Standard_M64ms (各 64 vCPU) = 128

>**注**: vCPU とメモリの要件を最小限に抑えるには、Standard_M64mではなく、Standard_M32ts (32 vCPU と 192 GiB のメモリ) を使用できます。
...
### タスク 1: ラボに十分な数の vCPU コアを検証する Windows を実行している Azure VM に SAP アーキテクチャを実装する
...
### タスク 2: Azure Center for SAP ソリューションを使用してラボの自動デプロイに十分な数の vCPU コアを検証する
...
1. **[確認および作成]** タブで、 **[作成]** を選択します。
    ...
    > **注**: Azure Center for SAP ソリューションを使用してラボの自動デプロイを開始する前に、クォータ制限の引き上げ要求が正常に完了するまで待ちます。
