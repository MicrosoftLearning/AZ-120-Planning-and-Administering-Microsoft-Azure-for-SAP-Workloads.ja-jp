...
#### タスク 4:Azure Bastion をプロビジョニングする 
...
1. Cloud Shell ペインの PowerShell セッションから次を実行して、この演習で前に作成した **az12001a-RG-vnet** という名前の仮想ネットワークに **AzureBastionSubnet** という名前のサブネットを追加します。
...
1. **[Bastion の作成]** ウィンドウの **[基本]** タブで、次の設定を指定して、**[確認および作成]** を選びます。
   ...
   |パブリック IP アドレス|**新規作成**|
   |パブリック IP の名前|**adVNET-ip**|

1. **[Bastion の作成]** ウィンドウの **[確認と作成]** タブで、**[作成]** を選択します。

   > **注**:デプロイが完了するまで待たずに、この演習の次のタスクに進みます。 デプロイには約 5 分かかります。
...
1. **az12001b-cl-vm0** のブレードで **[接続]** を選び、ドロップダウン メニューで **[Connect via Bastion] (Bastion 経由で接続)** を選び、**az12001b-cl-vm0** の **[Bastion]** タブで **[認証の種類]** を **[VM パスワード]** に設定したままにします。**az12001b-cl-vm0** 仮想マシンをデプロイするときに設定した資格情報を入力し、チェックボックス **[新しいブラウザー タブで開く]** をオンのままにして、**[接続]** を選びます。

    > **注**: オペレーティング システム レベルのアカウントではなく、必ず、**ADATUM** ドメイン アカウントを使用してサインインしてください (つまり、ユーザー名の前に **ADATUM\\** プレフィックスが付いていることを確かめます)。

1. az12001b-cl-vm0 への Bastion セッション内で、サーバー マネージャーの **[ファイルおよび記憶域サービス]** -> **[サーバー]** ノードに移動します。 
...
1. az12001b-cl-vm0 への Bastion セッション内で、Windows PowerShell ISE セッションを開始し、次のコマンドを実行して、az12001b-cl-vm0 と az12001b-cl-vm1 の両方にフェールオーバー クラスタリングとリモート管理ツールの機能をインストールします。
...
1. **az12001b-cl-vm0** のブレードから、Azure Bastion を使って仮想マシン ゲスト オペレーティング システムに接続します。 認証を求められたら、このラボの最初の演習で Bicep テンプレートをデプロイするときに指定した管理ユーザー アカウントの資格情報を指定します。

1. az12001b-cl-vm0 への Bastion セッションで、サーバー マネージャーの **[ツール]** メニューから、**Active Directory 管理センター**を開始します。
...
1. az12001b-cl-vm0 への Bastion セッション内で、Windows PowerShell ISE セッションを開始し、次のコマンドを実行して新しいクラスターを作成します。
...
1. az12001b-cl-vm0 への Bastion セッション内で、**Active Directory 管理センター** コンソールに切り替えます。
...
1. 結果の構成を確認するには、az12001b-cl-vm0 への Bastion セッション内で、サーバー マネージャーの **[ツール]** メニューから **[フェールオーバー クラスター マネージャー]** を起動します。

1. **[フェールオーバー クラスター マネージャー]** コンソールで、**az12001b-cl-cl0** クラスター構成 (ノードを含む)、監視設定およびネットワーク設定を確認します。 クラスターには共有ストレージがないことに注意してください。

1. az12001b-cl-vm0 への Bastion セッションを終了します。
...
1. Azure portal で、**az12001b-cl-vm0** Azure VM のブレードに移動します。

1. **az12001b-cl-vm0** のブレードから、**[ネットワーク]** ブレードに移動します。 

1. **[az12001b-cl-vm0 - ネットワーク]** ブレードから、az12001b-cl-vm0 のネットワーク インターフェイスに移動します。 

1. az12001b-cl-vm0 のネットワーク インターフェースのブレードから、その IP 構成ブレードに移動し、そこから **ipconfig1** ブレードを表示します。

1. **ipconfig1** ブレードで、プライベート IP アドレスの割り当てを **Static** に設定し、変更を保存します。

1. Azure portal で、**az12001b-cl-vm1** Azure VM のブレードに移動します。

1. **az12001b-cl-vm1** のブレードから、**[ネットワーク]** ブレードに移動します。 

1. **[az12001b-cl-vm1 - ネットワーク]** ブレードから、az12001b-cl-vm1 のネットワーク インターフェイスに移動します。 

1. az12001b-cl-vm1 のネットワーク インターフェースのブレードから、その IP 構成ブレードに移動し、そこから **ipconfig1** ブレードを表示します。
