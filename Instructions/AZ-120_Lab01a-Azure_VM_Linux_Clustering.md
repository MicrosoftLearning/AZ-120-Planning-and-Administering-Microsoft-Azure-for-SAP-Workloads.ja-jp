...
1. **[az12001a-vm0]** ブレードで **[接続]** を選択し、ドロップダウン メニューで **[Connect via Bastion] (Bastion 経由で接続)** を選択し、**[az12001a-vm0]** の **[Bastion]** タブで **[認証の種類]** を **[VM パスワード]** に設定したままにします。**az12001a-vm0** 仮想マシンをデプロイするときに設定した資格情報を入力し、チェックボックス **[新しいブラウザー タブで開く]** を有効なままにして、**[接続]** を選択します。
...
1. **az12001a-vm0** Azure VM への Bastion セッション内で、次を実行してパスフレーズレス SSH キーを生成します。
...
1. **az12001a-vm1** Azure VMへの Bastion セッションに切り替え、次を実行して vi エディターでファイル **/root/.ssh/authorized\_keys** を作成します (他の任意のエディターを使用してもかまいません)。
...
1. **az12001a-vm1** Azure VM への Bastion セッション内で、次を実行してパスフレーズレス SSH キーを生成します。
...
1. **az12001a-vm0** Azure VMへの Bastion セッションに切り替え、次を実行して vi エディターでファイル **/root/.ssh/authorized\_keys** を作成します (他の任意のエディターを使用してもかまいません)。
...
1. **az12001a-vm1** Azure VMへの Bastion セッションに切り替え、次を実行して vi エディターでファイル **/root/.ssh/authorized\_keys** を開きます (他の任意のエディターを使用してもかまいません)。
...
1. **az12001a-vm0** Azure VMへの Bastion セッションに切り替え、次を実行して vi エディターでファイル **/root/.ssh/authorized\_keys** を開きます (他の任意のエディターを使用してもかまいません)。
...
1. **az12001a-vm0** Azure VMへの Bastion セッション内で、次を実行して vi エディターでファイル **/etc/ssh/sshd\_config** を開きます (他の任意のエディターを使用してもかまいません)。
...
1. **az12001a-vm0** Azure VM への Bastion セッション内で、次を実行して sshd デーモンを再起動します。
...
1. 構成が正常に完了したことを検証するために、**az12001a-vm0** Azure VM への Bastion セッションで、次を実行して az12001a-vm0 から az12001a-vm1 への SSH セッションを **root** として確立します。 
...
1. 構成が正常に完了したことを検証するために、**az12001a-vm1** への Bastion セッションで、次を実行して az12001a-vm1 から az12001a-vm0 への SSH セッションを **root** として確立します。 
